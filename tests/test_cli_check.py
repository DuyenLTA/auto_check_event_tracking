...
from usv import cli_check, logcat_stream
...
def test_stdout_dung_MOT_dong_JSON(adb, tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(cli_check, "make_client", lambda: adb)

    code = cli_check.main(["--spec-tsv", str(spec_file), "--package", PACKAGE,
...
def test_chay_duoc_tren_console_cp1252(tmp_path):
    ...
    env = {**os.environ, "PYTHONIOENCODING": "cp1252"}
        ...
        [_sys.executable, "-m", "usv.cli_check", "--help"],
        # Con ep UTF-8 ra stream (xem cli_check.ep_utf8), nen cha phai doc
